...
public class InquiryAPIController extends BaseAPIController {
	...
	public void Inquiry_Department_Info() {
		...
		String sql = "SELECT a.id as departmentID,a.title as departmentName FROM stp_floor as a where a.parent_id=0 order by a.id";
		List<Record> area = Db.find(sql);
		List<Record> building=null;
		List<Record> floor=null;
		for(int i=0;i<area.size();i++) {
		
			String sql1 = "SELECT a.id as building,a.title as building_name FROM stp_floor as a where a.parent_id=? order by a.id";
			building = Db.find(sql1,area.get(i).getInt("departmentID"));
			for(int j=0;j<building.size();j++) {
				String sql2 = "SELECT a.id as floor,a.title as floor_name FROM stp_floor as a where a.parent_id=? order by a.id";
